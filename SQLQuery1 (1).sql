...
-- Tao bang hinh thuc muon
	...
	MaKM char(8) primary key check (MaKM in ('MV', 'DTC')),
...
-- Kiểm tra ngày thực trả
    ...
    END;
go
-- Tạo ràng buộc cập nhật tình trạng khi mượn tài liệu
create trigger CapNhatMuonTL
on PhieuMuon
after insert,update
as
begin
	...
	set MaTT= 'Bor'
	from TaiLieu t
	join CTPM ctpm on t.MaTL=ctpm.MaTL
	join inserted pm on ctpm.MaPM=pm.MaPM
	where pm.NgayMuon is not null and ctpm.NgayThucTra is null;
end;
go
-- Tạo ràng buộc cập nhật tình trạng khi trả tài liệu
create trigger CapNhatTraTL
on CTPM
after insert, update
as 
begin
	...
	set MaTT= 'Av'
	from TaiLieu t
	join inserted ctpm on t.MaTL=ctpm.MaTL
	where ctpm.NgayThucTra is not null;
end;
go

--Nhap du lieu
Insert into LoaiTL(MaTheLoai,TenTheloai)
Values
 ('GT', N'Giáo trình'),
 ('TC', N'Tạp chí');
